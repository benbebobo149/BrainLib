      ...
      <div class="h-3/4 flex bg-slate-50 p-6 items-start justify-end">
        ...
        <div class="h-full flex flex-col">
          ...
          <div class="h-2/3 flex-col justify-start">
            <p class="text-black " :style="{ fontSize: fontSize[0] }" contenteditable @input="updateContent(0)">Enter
              Title</p>
            <p class="text-black mt-10" :style="{ fontSize: fontSize[1] }" contenteditable @input="updateContent(1)">Enter
              Address</p>
            <p class="text-black mt-10" :style="{ fontSize: fontSize[2] }" contenteditable @input="updateContent(2)">Enter
              Description</p>
          </div>
...
const updateContent = (index) => {
  ...
  // Update font size based on the length of the entered content
  fontSize.value[index] = content.length > 20 ? "0.8rem" : "1rem";
